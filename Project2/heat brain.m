...
%% 
% Set the temperature on the left edge to 100 degrees. On the right edge,
% there is a prescribed heat flux out of the block. The top and bottom
% edges and the edges inside the cavity are all insulated, that is, no heat
...
%%
% Specify the thermal conductivity of the material. First, consider the
% constant thermal conductivity, for example, equal one. Later, consider a
% case where the thermal conductivity is a function of temperature.
...
%%
% Specify thermal condictivity,  mass density, and specific heat of the
% material.
...
%%
% Define boundary conditions. In the transient cases, the temperature on
% the left edge is zero at time=0 and ramps to 100 degrees over .5 seconds.
% On the right edge, there is a prescribed heat flux out of the block. The
% top and bottom edges as well as the edges inside the cavity are all
% insulated, that is no heat is transferred across these edges.
%
...
%%
% Calculate the transient solution. Perform a transient analysis from zero
% to five seconds. The toolbox saves the solution every .1 seconds so that
% plots of the results as functions of time can be created.
tlist = 0:.1:5;
...
% dependent variables. For example, assume that the thermal conductivity is
% a simple linear function of temperature:

k = @(~,state) 0.3+0.003*state.u;
%%
% In this case, the variable u is the temperature. For this example, assume
% that the density and specific heat are not functions of temperature.

thermalProperties(thermalmodelS,'ThermalConductivity',k);
%%
% Calculate the steady-state solution. Compared with the
% constant-conductivity case, the temperature on the right-hand edge is
% lower. This is due to the lower conductivity in regions with lower
% temperature.
...
% Now perform a transient analysis with the temperature-dependent
% conductivity.
...
%%
% Plot the temperature at the final time step and the temperature at the
% right edge as a function of time. The plot of temperature at the final
% time step is only slightly different from the comparable plot from the
% linear analysis: temperature at the right edge is slightly lower than the
% linear case. The plot of temperature as a function of time is
% considerably different from the linear case. Because of the lower
% conductivity at lower temperatures, the heat takes longer to reach the
% right edge of the block. In the linear case, the temperature is
% essentially constant at around three seconds but for this nonlinear case,
